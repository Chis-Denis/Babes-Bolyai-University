...
sigma=5;%case sigma 
%z-test
%[h,p,ci,zval] = ttest(X,m0,sigma,alfa,tail)
%alfa = def. 0.05
%h-0->H0 not rej.
% -1->H0 is rej.
%P-value
...
%        1 or 'right'
tail=1;
[h,p,ci,stats] = ttest(X,m0,alfa,tail);

if h==0
    fprintf('H0 is not rej., i.e on average, the number of files exceeds 5.5\n');
else
    fprintf('H0 is rej., i.e on average, the number of files does not exceed 5.5 \n');
end

fprintf('Observed value of the test statistic is %1.5f\n',stats.tstat);
fprintf('P value is %1.5f\n',p);
q2 = tinv(1-alfa,stats.df);
